...
import za.co.ntier.api.model.I_ZZLearnerQCTOArtisansAssessments;
...
import za.co.ntier.api.model.X_ZZLearnerQCTOArtisansAssessments;
...
public class LearnerAssessmentVM extends StepAppVM{
	...
	void showAssessmentSteo(int id) {
		...
		else if (isInterventionQCTOArtisans())
		{
			if (tmQctoArtisansAssessments == null)
			{
				initQctoArtisansAssessments();
			}

			int[] moduleIds = DB.getIDsEx(null, String.format(	"SELECT %s FROM %s WHERE %s = ?", X_ZZQctoModule.COLUMNNAME_ZZQctoModule_ID,
																X_ZZQctoLearnershipModule.Table_Name,
																X_ZZQctoLearnershipModule.COLUMNNAME_ZZQctoLearnership_ID), learnerQCTOArtisans
			...
			Query learnerAssessmentsQuery = MTable.get(Env.getCtx(), I_ZZLearnerQCTOArtisansAssessments.Table_Name).createQuery(
																																String.format(	"%s IN (%s) AND %s = ?",
																																				X_ZZLearnerQCTOArtisansAssessments.COLUMNNAME_ZZQctoModule_ID,
																																				placeholders,
																																				X_ZZLearnerQCTOArtisansAssessments.COLUMNNAME_ZZLearnerQCTOArtisans_ID),
																																null);

			List<Object> learnerAssessmentsParas = new ArrayList<Object>(ids);
			learnerAssessmentsParas.add(learnerQCTOArtisans.getZZLearnerQCTOArtisans_ID());
			learnerAssessmentsQuery.setParameters(learnerAssessmentsParas);
			List<PO> learnerAssessments = learnerAssessmentsQuery.list();

			List<List<PO>> loadSavedDatas = RowData.mergedList(modules, learnerAssessments, (po1, po2) -> {
				X_ZZQctoModule module = (X_ZZQctoModule) po1;
				X_ZZLearnerQCTOArtisansAssessments learnerAssessment = (X_ZZLearnerQCTOArtisansAssessments) po2;
				return learnerAssessment.getZZQctoModule_ID() != 0 && learnerAssessment.getZZQctoModule_ID() == module.getZZQctoModule_ID();
			});

			tmQctoArtisansAssessments.resetMultiPo(loadSavedDatas);
		}
		else if (isInterventionQCTOSkills())
		{
	...
	ColumnModel tradeTestNumberCol;
	ColumnModel tradeTestDateCol;
	ColumnModel nambConfirmationDateCol;
	CheckboxColumnModel nambConfirmationCol;
	...
	public void initAssessmentParam() {
		...
		if (isInterventionQCTOArtisans())
		{
			tradeTestNumberCol = CellModel.getColModelForGenericCell(	"Trade Test Number", I_ZZLearnerQCTOArtisansAssessments.COLUMNNAME_Trade_Test_Number,
																		CellModel.POSITIVE_NUM_CELL).setTableName(
																													I_ZZLearnerQCTOArtisansAssessments.Table_Name);
			cols.add(tradeTestNumberCol);

			tradeTestDateCol = DateCellModel.getDateColumnModel("Trade Test Date", I_ZZLearnerQCTOArtisansAssessments.COLUMNNAME_ZZTradeTestDate).setTableName(
																																								I_ZZLearnerQCTOArtisansAssessments.Table_Name);
			cols.add(tradeTestDateCol);

			nambConfirmationDateCol = DateCellModel.getDateColumnModel(	"NAMB Confirmation Date",
																		I_ZZLearnerQCTOArtisansAssessments.COLUMNNAME_Namb_Confirmation_Date).setTableName(
																																							I_ZZLearnerQCTOArtisansAssessments.Table_Name);
			cols.add(nambConfirmationDateCol);

			nambConfirmationCol = CheckboxCellModel.getCheckboxColModel("NAMB Confirmation", I_ZZLearnerQCTOArtisansAssessments.COLUMNNAME_Namb_Confirmation);
			nambConfirmationCol.setTableName(I_ZZLearnerQCTOArtisansAssessments.Table_Name);
			nambConfirmationCol.setShowTitle(false);
			cols.add(nambConfirmationCol);

			competentCol = CheckboxCellModel.getCheckboxColModel("Competent", null);
			competentCol.setShowTitle(false);
			cols.add(competentCol);

			BiConsumer<Event, CellModel> tradeTestHandler = (event, cellModel) -> {
				CellModel tradeNum = cellModel.getRowModel().get(tradeTestNumberCol);
				CellModel tradeDate = cellModel.getRowModel().get(tradeTestDateCol);
				boolean hasTradeNum = tradeNum != null && tradeNum.getValue() != null && !tradeNum.getValue().toString().trim().isEmpty();
				boolean hasTradeDate = tradeDate != null && tradeDate.getValue() != null;
				if (hasTradeNum || hasTradeDate)
				{
					DateCellModel dateCell = (DateCellModel) cellModel.getRowModel().get(nambConfirmationDateCol);
			...
			tradeTestNumberCol.setEventHandle(tradeTestHandler);
			tradeTestDateCol.setEventHandle(tradeTestHandler);

			BiConsumer<Event, CellModel> nambHandler = (event, cellModel) -> {
				CellModel nambDate = cellModel.getRowModel().get(nambConfirmationDateCol);
				CheckboxCellModel nambConf = (CheckboxCellModel) cellModel.getRowModel().get(nambConfirmationCol);
				boolean hasNambDate = nambDate != null && nambDate.getValue() != null;
				boolean hasNambConf = nambConf != null && nambConf.isChecked();
				if (hasNambDate || hasNambConf)
				{
					CellModel tradeNum = cellModel.getRowModel().get(tradeTestNumberCol);
			...
			nambConfirmationDateCol.setEventHandle(nambHandler);
			nambConfirmationCol.setEventHandle(nambHandler);

			nambConfirmationDateCol.setValidateHandle((cellModel, validateMsgs) -> {
				CheckboxCellModel nambConfCell = (CheckboxCellModel) cellModel.getRowModel().get(nambConfirmationCol);
				if (nambConfCell != null && nambConfCell.isChecked())
				{
					if (cellModel.getValue() == null)
					{
						validateMsgs.add("NAMB Confirmation Date is mandatory when NAMB Confirmation is checked");
					}
				}
			});
			...
			tmAssessmentParam = TableModel.getTableBean(TableModel.class, cols, false, I_ZZLearnerQCTOArtisansAssessments.Table_Name);
			tmAssessmentParam.setViewModel(ViewType.VIEW_FORM);
			tmAssessmentParam.setSclass("srd-LearnerAssessment-artisanAssessmentParam");
		...
		competentCol.setEventHandle((event, cellModel) -> {
			CheckboxCellModel competentCell = (CheckboxCellModel) cellModel;
		...
		rplcol.setEventHandle((event, cellModel) -> {
			CheckboxCellModel rplCell = (CheckboxCellModel) cellModel;
			if (rplCell.isChecked()) {
		...
		String whereAssessor = buildAssessorSearchQuery(X_ZZAssessorPerson_v.ZZASSESSORROLE_Assessor);
		String whereModerator = buildAssessorSearchQuery(X_ZZAssessorPerson_v.ZZASSESSORROLE_Moderator);
	...
	private String buildAssessorSearchQuery(String role) {
		...
		String qualificationLink = getAssessorQualificationFilter();

		return String.format("%s='%s'%s%s", 
				I_ZZAssessorPerson_v.COLUMNNAME_ZZAssessorRole, role, activeApproved, qualificationLink);
	}

	private String getAssessorQualificationFilter() {
		if (isInterventionQCTOLearnerships() && qctoLearnership != null) {
			if (qctoLearnership.getZZQctoQualification_ID() > 0) {
				return buildQualificationExistsQuery(
			...
			if (learnership.getZZQualification_ID() > 0) {
				return buildQualificationExistsQuery(
					I_ZZLinkAssessorQualification_v.Table_Name,
					I_ZZLinkAssessorQualification_v.COLUMNNAME_ZZAssessorPerson_ID,
		...
		// If no qualification is set, force the query to return empty results.
		return " AND 1=0 ";
	}

	private String buildQualificationExistsQuery(String linkTableName, String linkPersonColumn, String linkQualColumn, int qualId) {
		return String.format(" AND EXISTS (SELECT 1 FROM %s q WHERE q.%s = %s.%s AND q.%s = %d)",
			linkTableName,
			linkPersonColumn,
			I_ZZAssessorPerson_v.Table_Name,
	...
	private ColumnModel getAssessmentStatusColModel(String tableName, String columnName) {
		ValueAdaptColumnModel col = ValueAdaptCellModel.getValueAdaptColumnModel(
			MasterUtil.getNameOfColTranslated(tableName, columnName),
			columnName,
			CellModel.LABEL_CELL
		);
		col.setValueFromDaoAdaptHandle(value -> {
			if (X_ZZLearnerQctoLearnershipAssessments.ZZASSESSMENTSTATUS_Competent.equals(value)) {
				return "Competent";
			} else if (X_ZZLearnerQctoLearnershipAssessments.ZZASSESSMENTSTATUS_NotCompetent.equals(value)) {
	...
	private ColumnModel getPersonColModel(String tableName, String columnName) {
		String headerName = MasterUtil.getNameOfColTranslated(tableName, columnName);
		if (columnName.contains("Assessor")) {
			headerName = "Assessor";
		} else if (columnName.contains("Moderator")) {
	...
	void initQctoLearnershipAssessments() {
		List<ColumnModel> cols = new ArrayList<>();
		...
		col = getAssessmentStatusColModel(I_ZZLearnerQctoLearnershipAssessments.Table_Name, I_ZZLearnerQctoLearnershipAssessments.COLUMNNAME_ZZAssessmentStatus);
		cols.add(col);
		
		col = CellModel.getColModelForLabel(
				MasterUtil.getNameOfColTranslated(I_ZZQctoModule.Table_Name, I_ZZQctoModule.COLUMNNAME_ZZCredits)
				, I_ZZQctoModule.COLUMNNAME_ZZCredits
				).setTableName(I_ZZQctoModule.Table_Name);
		cols.add(col);
		...
		col = getPersonColModel(I_ZZLearnerQctoLearnershipAssessments.Table_Name, I_ZZLearnerQctoLearnershipAssessments.COLUMNNAME_Assessor_ID);
		cols.add(col);
		...
		col = getPersonColModel(I_ZZLearnerQctoLearnershipAssessments.Table_Name, I_ZZLearnerQctoLearnershipAssessments.COLUMNNAME_Moderator_ID);
		cols.add(col);
		...
		tmQctoLearnershipAssessments.setRowStyleLogic(row -> {
			X_ZZLearnerQctoLearnershipAssessments a = (X_ZZLearnerQctoLearnershipAssessments) row.getRowData().getDataNullable(I_ZZLearnerQctoLearnershipAssessments.Table_Name);
			return a != null && X_ZZLearnerLearnershipAssessments.ZZASSESSMENTSTATUS_Competent.equals(a.getZZAssessmentStatus()) ? "background-color: rgba(40, 167, 69, 0.2);" : "";
		});
		...
		tmQctoLearnershipAssessments.setBeforeSave((rowDbEventArgs) -> {
			...
			else
			{
				...
				assessment.setAssessor_ID(assessorId);
				int[] assessorIds = PO.getAllIDs("ZZAssessorPerson_v", "AD_User_ID=" + assessorId, null);
				if (assessorIds.length > 0)
				{
					X_ZZAssessorPerson_v assessor = new X_ZZAssessorPerson_v(Env.getCtx(), assessorIds[0], null);
			...
			if (competentCell.isChecked()) {
				boolean isCompetent = X_ZZLearnerQctoLearnershipAssessments.ZZASSESSMENTSTATUS_Competent.equals(assessment.getZZAssessmentStatus());
				if (isCompetent) {
					assessment.setZZAssessmentStatus(X_ZZLearnerQctoLearnershipAssessments.ZZASSESSMENTSTATUS_NotCompetent);
					assessment.setZZRPL(X_ZZLearnerQctoLearnershipAssessments.ZZRPL_No);
				} else {
					assessment.setZZAssessmentStatus(X_ZZLearnerQctoLearnershipAssessments.ZZASSESSMENTSTATUS_Competent);
				}
			} else if (rplCell.isChecked()) {
				boolean isRpl = X_ZZLearnerQctoLearnershipAssessments.ZZRPL_Yes.equals(assessment.getZZRPL());
				if (isRpl) {
					assessment.setZZRPL(X_ZZLearnerQctoLearnershipAssessments.ZZRPL_No);
					assessment.setZZAssessmentStatus(X_ZZLearnerQctoLearnershipAssessments.ZZASSESSMENTSTATUS_NotCompetent);
				} else {
					assessment.setZZRPL(X_ZZLearnerQctoLearnershipAssessments.ZZRPL_Yes);
					assessment.setZZAssessmentStatus(X_ZZLearnerQctoLearnershipAssessments.ZZASSESSMENTSTATUS_Competent);
				}
			}
			...
			if (X_ZZLearnerQctoLearnershipAssessments.ZZASSESSMENTSTATUS_Competent.equals(assessment.getZZAssessmentStatus())) {
	...
	CheckboxColumnModel qctoArtisansAssessmentsSelectedCol;

	public void initQctoArtisansAssessments()
	{
		List<ColumnModel> cols = new ArrayList<>();

		qctoArtisansAssessmentsSelectedCol = CheckboxCellModel.getCheckboxColModel("", null);
		cols.add(qctoArtisansAssessmentsSelectedCol);
		...
		ColumnModel statusCol = getAssessmentStatusColModel(I_ZZLearnerQCTOArtisansAssessments.Table_Name,
															I_ZZLearnerQCTOArtisansAssessments.COLUMNNAME_ZZAssessmentStatus);
		cols.add(statusCol);
		...
		col = CellModel.getColModelForLabel("Trade Test No.", I_ZZLearnerQCTOArtisansAssessments.COLUMNNAME_Trade_Test_Number).setTableName(
																																			I_ZZLearnerQCTOArtisansAssessments.Table_Name);
		cols.add(col);

		col = DateCellModel	.getDateColumnModel("Trade Test Date", I_ZZLearnerQCTOArtisansAssessments.COLUMNNAME_ZZTradeTestDate).setTableName(
																																				I_ZZLearnerQCTOArtisansAssessments.Table_Name)
							.setReadonly(true);
		cols.add(col);

		col = getBooleanColModel("NAMB Conf.", I_ZZLearnerQCTOArtisansAssessments.Table_Name, I_ZZLearnerQCTOArtisansAssessments.COLUMNNAME_Namb_Confirmation);
		cols.add(col);

		col = DateCellModel	.getDateColumnModel("NAMB Conf. Date", I_ZZLearnerQCTOArtisansAssessments.COLUMNNAME_Namb_Confirmation_Date).setTableName(
																																						I_ZZLearnerQCTOArtisansAssessments.Table_Name)
							.setReadonly(true);
		cols.add(col);

		tmQctoArtisansAssessments = TableModel.getTableBean(TableModel.class, cols, false, I_ZZLearnerQCTOArtisansAssessments.Table_Name);
		tmQctoArtisansAssessments.setViewModel(ViewType.VIEW_GRID);
		tmQctoArtisansAssessments.setSclass("srd-LearnerAssessment-qctoArtisansAssessments");
		tmQctoArtisansAssessments.setRowStyleLogic(row -> {
			X_ZZLearnerQCTOArtisansAssessments a = (X_ZZLearnerQCTOArtisansAssessments) row.getRowData().getDataNullable(
																															I_ZZLearnerQCTOArtisansAssessments.Table_Name);
			return a != null && X_ZZLearnerQCTOArtisansAssessments.ZZASSESSMENTSTATUS_Competent.equals(a.getZZAssessmentStatus())
																																	? "background-color: rgba(40, 167, 69, 0.2);"
																																	: "";
		});

		tmQctoArtisansAssessments.setRowSaveFilter(rowMode -> {
			CheckboxCellModel selectionCell = (CheckboxCellModel) rowMode.get(qctoArtisansAssessmentsSelectedCol);
			return selectionCell.isChecked();
		});

		tmQctoArtisansAssessments.setBeforeSave((rowDbEventArgs) -> {
			if (!rowDbEventArgs.isRowEven())
				return true;

			X_ZZLearnerQCTOArtisansAssessments assessment = (X_ZZLearnerQCTOArtisansAssessments) rowDbEventArgs.row().getRowData().getDataNewWhenNull(
																																						I_ZZLearnerQCTOArtisansAssessments.Table_Name);

			if (assessment.getZZLearnerQCTOArtisans_ID() == 0)
			{
				assessment.setZZLearnerQCTOArtisans_ID(learnerQCTOArtisans.getZZLearnerQCTOArtisans_ID());
			}

			X_ZZQctoModule module = (X_ZZQctoModule) rowDbEventArgs.row().getRowData().getDataNullable(I_ZZQctoModule.Table_Name);
			if (assessment.getZZQctoModule_ID() == 0 && module != null)
			{
				assessment.setZZQctoModule_ID(module.getZZQctoModule_ID());
			}

			CheckboxCellModel compCell = (CheckboxCellModel) tmAssessmentParam.getRow().get(competentCol);
			if (compCell != null && compCell.isChecked())
			{
				boolean isCompetent = X_ZZLearnerQCTOArtisansAssessments.ZZASSESSMENTSTATUS_Competent.equals(assessment.getZZAssessmentStatus());
				if (isCompetent)
				{
					assessment.setZZAssessmentStatus(X_ZZLearnerQCTOArtisansAssessments.ZZASSESSMENTSTATUS_NotCompetent);
				}
				else
				{
					assessment.setZZAssessmentStatus(X_ZZLearnerQCTOArtisansAssessments.ZZASSESSMENTSTATUS_Competent);
				}
			}

			CellModel tradeTestCell = (CellModel) tmAssessmentParam.getRow().get(tradeTestNumberCol);
			DateCellModel tradeTestDateCell = (DateCellModel) tmAssessmentParam.getRow().get(tradeTestDateCol);
			DateCellModel dateCell = (DateCellModel) tmAssessmentParam.getRow().get(nambConfirmationDateCol);
			CheckboxCellModel nambConfCell = (CheckboxCellModel) tmAssessmentParam.getRow().get(nambConfirmationCol);

			boolean hasTradeNum = tradeTestCell != null && tradeTestCell.getValue() != null && !tradeTestCell.getValue().toString().trim().isEmpty();
			boolean hasTradeDate = tradeTestDateCell != null && tradeTestDateCell.getTimestamp() != null;
			boolean hasNambDate = dateCell != null && dateCell.getTimestamp() != null;
			boolean hasNambConf = nambConfCell != null && nambConfCell.isChecked();
			...
			if (hasTradeNum || hasTradeDate)
			{
				...
				if (hasTradeDate)
				{
					assessment.setZZTradeTestDate(tradeTestDateCell.getTimestamp());
				}
				assessment.setNamb_Confirmation_Date(null);
				assessment.setNamb_Confirmation(false);
			}
			else if (hasNambDate || hasNambConf)
			{
				assessment.setTrade_Test_Number(0);
				assessment.setZZTradeTestDate(null);
				if (hasNambDate)
				{
					assessment.setNamb_Confirmation_Date(dateCell.getTimestamp());
				}
				if (hasNambConf)
				{
					assessment.setNamb_Confirmation(true);
				}
			}
			if (assessment.getZZDateAssessmentCaptured() == null)
			{
	...
	public void initQctoSkillsAssessments()
	{
		...
		col = getAssessmentStatusColModel(I_ZZLearnerQCTOSkillsProgrammeAssessments.Table_Name, I_ZZLearnerQCTOSkillsProgrammeAssessments.COLUMNNAME_ZZAssessmentStatus);
		cols.add(col);
		...
		col = getPersonColModel(I_ZZLearnerQCTOSkillsProgrammeAssessments.Table_Name, I_ZZLearnerQCTOSkillsProgrammeAssessments.COLUMNNAME_Moderator_ID);
		cols.add(col);
		...
		tmQCTOSkillsAssessments.setSclass("srd-LearnerAssessment-qctoSkillsProgrammeAssessments");
		tmQCTOSkillsAssessments.setRowStyleLogic(row -> {
			X_ZZLearnerQCTOSkillsProgrammeAssessments a = (X_ZZLearnerQCTOSkillsProgrammeAssessments) row.getRowData().getDataNullable(I_ZZLearnerQCTOSkillsProgrammeAssessments.Table_Name);
			return a != null && X_ZZLearnerQCTOSkillsProgrammeAssessments.ZZASSESSMENTSTATUS_Competent.equals(a.getZZAssessmentStatus()) ? "background-color: rgba(40, 167, 69, 0.2);" : "";
		});
		...
		tmQCTOSkillsAssessments.setBeforeSave((rowDbEventArgs) -> {
			...
			CheckboxCellModel competentCell = (CheckboxCellModel) tmAssessmentParam.getRow().get(competentCol);
			if (competentCell.isChecked()) {
				boolean isCompetent = X_ZZLearnerQCTOSkillsProgrammeAssessments.ZZASSESSMENTSTATUS_Competent.equals(assessment.getZZAssessmentStatus());
				if (isCompetent) {
					assessment.setZZAssessmentStatus(X_ZZLearnerQCTOSkillsProgrammeAssessments.ZZASSESSMENTSTATUS_NotCompetent);
					assessment.setZZRPL(false);
				} else {
					assessment.setZZAssessmentStatus(X_ZZLearnerQCTOSkillsProgrammeAssessments.ZZASSESSMENTSTATUS_Competent);
				}
			} else if (rplCell.isChecked()) {
				boolean isRpl = assessment.isZZRPL();
				if (isRpl) {
					assessment.setZZRPL(false);
					assessment.setZZAssessmentStatus(X_ZZLearnerQCTOSkillsProgrammeAssessments.ZZASSESSMENTSTATUS_NotCompetent);
				} else {
					assessment.setZZRPL(true);
					assessment.setZZAssessmentStatus(X_ZZLearnerQCTOSkillsProgrammeAssessments.ZZASSESSMENTSTATUS_Competent);
				}
			}
			...
			X_ZZQctoModule qctoModule = (X_ZZQctoModule) rowDbEventArgs.row().getRowData().getDataNullable(I_ZZQctoModule.Table_Name);
			assessment.setZZQctoModule_ID(qctoModule.getZZQctoModule_ID());
			if (X_ZZLearnerQCTOSkillsProgrammeAssessments.ZZASSESSMENTSTATUS_Competent.equals(assessment.getZZAssessmentStatus())) {
				assessment.setZZCredits(qctoModule.getZZCredits());
			} else {
				assessment.setZZCredits(0);
			}
	...
	void initSkillsProgrammeAssessments() {
		List<ColumnModel> cols = new ArrayList<>();
		...
		col = getAssessmentStatusColModel(I_ZZLearnerSkillsProgrammeAssessments.Table_Name, I_ZZLearnerSkillsProgrammeAssessments.COLUMNNAME_ZZAssessmentStatus);
		cols.add(col);
		col = CellModel.getColModelForLabel(
		...
		col = getPersonColModel(I_ZZLearnerSkillsProgrammeAssessments.Table_Name, I_ZZLearnerSkillsProgrammeAssessments.COLUMNNAME_Moderator_ID);
		cols.add(col);
		...
		tmSkillsProgrammeAssessments.setSclass("srd-LearnerAssessment-skillsProgrammeAssessments");
		tmSkillsProgrammeAssessments.setRowStyleLogic(row -> {
			X_ZZLearnerSkillsProgrammeAssessments a = (X_ZZLearnerSkillsProgrammeAssessments) row.getRowData().getDataNullable(I_ZZLearnerSkillsProgrammeAssessments.Table_Name);
			return a != null && X_ZZLearnerSkillsProgrammeAssessments.ZZASSESSMENTSTATUS_Competent.equals(a.getZZAssessmentStatus()) ? "background-color: rgba(40, 167, 69, 0.2);" : "";
		});
		...
		tmSkillsProgrammeAssessments.setBeforeSave((rowDbEventArgs) -> {
			...
			CheckboxCellModel competentCell = (CheckboxCellModel) tmAssessmentParam.getRow().get(competentCol);
			if (competentCell.isChecked()) {
				boolean isCompetent = X_ZZLearnerSkillsProgrammeAssessments.ZZASSESSMENTSTATUS_Competent.equals(assessment.getZZAssessmentStatus());
				if (isCompetent) {
					assessment.setZZAssessmentStatus(X_ZZLearnerSkillsProgrammeAssessments.ZZASSESSMENTSTATUS_NotCompetent);
					assessment.setZZRPL(false);
				} else {
					assessment.setZZAssessmentStatus(X_ZZLearnerSkillsProgrammeAssessments.ZZASSESSMENTSTATUS_Competent);
				}
			} else if (rplCell.isChecked()) {
				boolean isRpl = assessment.isZZRPL();
				if (isRpl) {
			...
			if (X_ZZLearnerSkillsProgrammeAssessments.ZZASSESSMENTSTATUS_Competent.equals(assessment.getZZAssessmentStatus())) {
...
public void initLearnerLearnership()
	...
	public void initLearnershipAssessments()
	{
		...
		col = getAssessmentStatusColModel(I_ZZLearnerLearnershipAssessments.Table_Name, I_ZZLearnerLearnershipAssessments.COLUMNNAME_ZZAssessmentStatus);
		cols.add(col);
		...
		col = getPersonColModel(I_ZZLearnerLearnershipAssessments.Table_Name, I_ZZLearnerLearnershipAssessments.COLUMNNAME_Assessor_ID);
		cols.add(col);
		...
		col = getPersonColModel(I_ZZLearnerLearnershipAssessments.Table_Name, I_ZZLearnerLearnershipAssessments.COLUMNNAME_Moderator_ID);
		cols.add(col);
		...
		tmLearnershipAssessments.setSclass("srd-LearnerAssessment-learnershipAssessments");
		tmLearnershipAssessments.setRowStyleLogic(row -> {
			X_ZZLearnerLearnershipAssessments a = (X_ZZLearnerLearnershipAssessments) row.getRowData().getDataNullable(I_ZZLearnerLearnershipAssessments.Table_Name);
			return a != null && X_ZZLearnerLearnershipAssessments.ZZASSESSMENTSTATUS_Competent.equals(a.getZZAssessmentStatus()) ? "background-color: rgba(40, 167, 69, 0.2);" : "";
		});
		...
		tmLearnershipAssessments.setBeforeSave((rowDbEventArgs) -> {
			...
			{
				assessment.setAssessor_ID(0);
				assessment.set_ValueOfColumn(I_ZZLearnerLearnershipAssessments.COLUMNNAME_ZZ_SDPAdmin_ID, null);
			}
			else
			{
				int assessorId = (int) assessorSelected.getValue();
				...
				{
					X_ZZAssessorPerson_v assessor = new X_ZZAssessorPerson_v(Env.getCtx(), assessorIds[0], null);
					assessment.set_ValueOfColumn(I_ZZLearnerLearnershipAssessments.COLUMNNAME_ZZ_SDPAdmin_ID, assessor.getCreatedBy());
				}
			}
			...
			CheckboxCellModel competentCell = (CheckboxCellModel) tmAssessmentParam.getRow().get(competentCol);
			if (competentCell.isChecked()) {
				boolean isCompetent = X_ZZLearnerLearnershipAssessments.ZZASSESSMENTSTATUS_Competent.equals(assessment.getZZAssessmentStatus());
				if (isCompetent) {
					assessment.setZZAssessmentStatus(X_ZZLearnerLearnershipAssessments.ZZASSESSMENTSTATUS_NotCompetent);
					assessment.setZZRPL(X_ZZLearnerLearnershipAssessments.ZZRPL_No);
				} else {
					assessment.setZZAssessmentStatus(X_ZZLearnerLearnershipAssessments.ZZASSESSMENTSTATUS_Competent);
				}
			} else if (rplCell.isChecked()) {
				boolean isRpl = X_ZZLearnerLearnershipAssessments.ZZRPL_Yes.equals(assessment.getZZRPL());
				if (isRpl) {
					assessment.setZZRPL(X_ZZLearnerLearnershipAssessments.ZZRPL_No);
					assessment.setZZAssessmentStatus(X_ZZLearnerLearnershipAssessments.ZZASSESSMENTSTATUS_NotCompetent);
				} else {
					assessment.setZZRPL(X_ZZLearnerLearnershipAssessments.ZZRPL_Yes);
					assessment.setZZAssessmentStatus(X_ZZLearnerLearnershipAssessments.ZZASSESSMENTSTATUS_Competent);
				}
			}
			...
			if (X_ZZLearnerLearnershipAssessments.ZZASSESSMENTSTATUS_Competent.equals(assessment.getZZAssessmentStatus())) {
